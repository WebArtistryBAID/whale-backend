...
def get_item(id: int, db: Session = Depends(get_db)):
    return crud.assert_not_null(crud.get_item_type(db, id))
...
def get_category(id: int, db: Session = Depends(get_db)):
    return crud.assert_not_null(crud.get_category(db, id))


@app.get("/settings", response_model=str)
def get_setting(key: str, db: Session = Depends(get_db)):
    return crud.assert_not_null(crud.get_settings(db, key)).value


@app.get("/order", response_model=OrderSchema)
def get_order(id: int, db: Session = Depends(get_db)):
    return crud.assert_not_null(crud.get_order(db, id))
